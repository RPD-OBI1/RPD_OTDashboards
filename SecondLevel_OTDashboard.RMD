...
pal <- data.frame(color = brewer.pal(n = otdata %>% # Build color palette
                                         select(CostCenter) %>% 
                                         distinct() %>% 
                                         nrow(),
                                     name = "Paired"),
...
if(numofslips == 0) {
    ...
    earners <- otdata %>%
        ...
        arrange(desc(Amount)) %>%
        ungroup() %>%
        mutate(PersonnelName_New = as.factor(PersonnelName_New)) %>%
        left_join(pal, by = "CostCenter") %>%
        mutate(PersonnelName_New = fct_reorder(PersonnelName_New, Amount),
               ShowLegend = case_when(duplicated(.$CostCenter) ~ FALSE,
                                      TRUE ~ TRUE))
    ...
    p <- p %>%
        layout(yaxis = list(autorange = "reversed"),
               xaxis = list(tickprefix = "$", exponentformat = "none"),
               legend = list(traceorder = "grouped+normal"),
               margin = list(l = 150),
               barmode = "stack")
