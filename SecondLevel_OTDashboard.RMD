...
if(numofslips == 0) {
    ...
    p <- plot_ly(
        ...
        marker = list(color = pal[pal$CostCenter == names(activitytypes)[2], "color"]),
        orientation = "h")
    ...
    for (i in 3:(ncol(activitytypes) - 1)) {
        p <- add_trace(
            p,
            x = activitytypes[, i],
            y = activitytypes$ActivityGrouping,
            marker = list(color = pal[pal$CostCenter == names(activitytypes)[i] , "color"]),
...
if(numofslips == 0) {
    ...
    earners <- earners %>%
        ...
        mutate(ShowLegend = case_when(duplicated(.$CostCenter) ~ FALSE,
               ...
               PersonnelName_New = factor(PersonnelName_New, levels = earnersTotal$PersonnelName_New))
    ...
    for ( i in 1:length(unique(earners$CostCenter))) {
        q <- q %>%
            add_trace(earners[earners$CostCenter == unique(earners$CostCenter)[i],],
                  x = earners[earners$CostCenter == unique(earners$CostCenter)[i], ] %>% .$Amount,
                  y = earners[earners$CostCenter == unique(earners$CostCenter)[i], ] %>% .$PersonnelName_New,
                  name = unique(earners$CostCenter)[i],
                  marker = list(color = earners[earners$CostCenter == unique(earners$CostCenter)[i], ] %>% 
                                     .$color))
    }
...
if(numofslips == 0) {
    ...
    for (i in 1:length(unique(earners$CostCenter))) {
        q <- q %>%
            add_trace(earners[earners$CostCenter == unique(earners$CostCenter)[i],],
                      x = earners[earners$CostCenter == unique(earners$CostCenter)[i], ] %>% 
                          .$HoursWorked,
                      y = earners[earners$CostCenter == unique(earners$CostCenter)[i], ] %>% 
                          .$PersonnelName_New,
                      name = unique(earners$CostCenter)[i],
                      marker = list(color = earners[earners$CostCenter == unique(earners$CostCenter)[i], ] %>% 
                                        .$color))
