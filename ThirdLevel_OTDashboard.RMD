...
CostCentersSelected <- read_csv("Z:/Projects/dashboard/RPDOvertimeDashboard/FY1718_Budgets.csv") %>%
    ...
    mutate(Allotment = gsub(Allotment, pattern = ",", replacement = "") %>%
           ...
           LevelTwo = case_when(is.na(.$LevelTwo) ~ "OTHER",
                                TRUE ~ .$LevelTwo)) %>%
    select(CostCenter, Allotment, AllottedHours, LevelTwo)
...
if(numofslips == 0) {
    ...
    meetbudget2 <- otdata %>%
        ...
        mutate(LevelTwo = fct_reorder(LevelTwo, AmountSpent, .desc = TRUE))
...
if(numofslips == 0) {
    ...
    workplatoonL2 <- workplatoonL2 %>% 
        select(-HoursAmount) %>% 
        spread(WorkingPlatoon, Amount, drop = FALSE, fill = 0) %>%
...
if(numofslips == 0) {
    ...
    earners <- earners %>%
        ...
        mutate(PersonnelName_New = factor(PersonnelName_New, levels = earnersTotal$PersonnelName_New),
               PersonnelName_New = fct_reorder(PersonnelName_New, TotalAmount))
...
if(numofslips == 0) {
    ...
    q <- plot_ly(
        ...
        x = ~OTDate,
        y = ~Amount,
        text = ~paste0("$", Amount, " spent on ", OTDate, " on ", CostCenter),
        hoverinfo = "text",
        type = "scatter", 
        mode = "markers",
        color = ~CostCenter) %>%
...
if(numofslips == 0) {
    ...
    LevelTwoAllotments <- CostCentersSelected %>% 
        group_by(LevelTwo) %>% 
        summarise(LevelTwoAllotment = sum(AllottedHours, na.rm = TRUE)) %>%
        mutate(LevelTwo = case_when(is.na(.$LevelTwo) ~ "OTHER",
                                    TRUE ~ .$LevelTwo))
    ...
    meetbudget2 <- otdata %>%
        ...
        left_join(LevelTwoAllotments, by = "LevelTwo") %>%
        mutate(LevelTwo = as.factor(LevelTwo)) %>%
        mutate(LevelTwo = fct_reorder(LevelTwo, HoursWorked, .desc = TRUE))
    
    meetbudget2Total <- data.frame(
        LevelThree = params$LevelThreeName,
        HoursWorked = sum(meetbudget2$HoursWorked, na.rm = TRUE),
        AllottedHours = sum(meetbudget2$LevelTwoAllotment, na.rm = TRUE)
        )
...
p <- plot_ly(
    ...
    marker = list(color = "blue"),
    showlegend = FALSE) %>%
    add_trace(y = ~HoursWorked, name = "HoursWorked", marker = list(color = "red"), showlegend = FALSE) %>%
    layout(xaxis = list(title = ""),
...
q <- plot_ly(
    ...
    name = "Allotted",
    marker = list(color = "blue")) %>%
    add_trace(y = ~HoursWorked, name = "Worked", marker = list(color = "red")) %>%
    layout(xaxis = list(title = ""),
           yaxis = list(title = "Hours", exponentformat = "none"),
           barmode = "group",
           hovermode = "closest")
...
if(numofslips == 0) {
    ...
    workplatoonL2 <- workplatoonL2 %>% 
        select(-Amount) %>% 
        spread(WorkingPlatoon, HoursAmount, drop = FALSE, fill = 0) %>%
        rename(FirstPlatoon = `1`, 
               SecondPlatoon = `2`, 
               ThirdPlatoon = `3`)
...
if(numofslips == 0) {
    cat("No OT worked.")
        ...
        projectcodes <- otdata %>%
            ...
            ungroup() %>%
            mutate(ProjectCodeFactor = fct_reorder(ProjectCodeFactor, HoursWorked)) %>%
            spread(LevelTwo, HoursWorked, fill = 0) %>%
            mutate(sumCC = rowSums(select(., -ProjectCodeFactor), na.rm = TRUE)) %>%
            arrange(desc(sumCC)) %>%
            top_n(n = 10, wt = sumCC) %>%
            mutate(ProjectCodeFactor = fct_reorder(ProjectCodeFactor, sumCC)) %>% # drop unused factor levels
...
if(numofslips == 0) {
    ...
    activitytypes <- otdata %>%
        ...
        spread(key = CostCenter, value = HoursWorked, fill = 0) %>%
        ungroup() %>% 
        mutate(sumCC = rowSums(select(., one_of(CostCentersSelected$CostCenter)))) %>% # sumCC sums up the columns with names of cost centers
        as.data.frame()
    ...
    activitytypes <- otdata %>%
        ...
        summarise(HoursWorked = sum(OvertimeHours)) %>%
        mutate(HoursWorked = replace(HoursWorked, is.na(HoursWorked), 0)) %>% # replace NA values with 0
        spread(key = LevelTwo, value = HoursWorked, fill = 0) %>%
        ungroup() %>% 
        mutate(sumCC = rowSums(select(., -ActivityGrouping), na.rm = TRUE)) %>% # sumCC sums up the columns with names of cost centers
        as.data.frame()
...
# Need a way to get the unused payperiods in there
    ...
    plotpayperiod <- anti_join(payperiod2017lookup, payperiod, by = "PayPeriod") %>%
        ...
        bind_rows(payperiod) %>%
        arrange(PayPeriod) %>%
        replace_na(replace = list(TotalHours = 0))
...
if(numofslips == 0) {
    ...
    q <- plot_ly(
        ...
        mode = "markers",
        hoverinfo = "text",
        text = ~paste0(HoursWorked, " hours worked on ", OTDate, " for ", CostCenter),
        color = ~CostCenter) %>%
        layout(legend = list(orientation = "h"),
               hovermode = "closest",
               yaxis = list(title = "Hours", 
    ...
    q <- plot_ly(
        ...
        mode = "markers",
        hoverinfo = "text",
        text = paste0(byday$HoursWorked, " hours worked on ", byday$OTDate, " by ", byday$LevelTwo),
        color = ~LevelTwo) %>%
        layout(hovermode = "closest",
               legend = list(orientation = "h"),
               yaxis = list(title = "Hours", 
